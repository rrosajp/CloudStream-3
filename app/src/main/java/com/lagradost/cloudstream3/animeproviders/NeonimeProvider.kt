...
class NeonimeProvider : MainAPI() {
    ...
    override suspend fun getMainPage(): HomePageResponse {
        ...
        document.select("div.item_1.items,div#slid01").forEach { block ->
            val header =
                block.previousElementSibling()?.select("h1")?.text() ?: block.selectFirst("h3")
                    ?.text().toString()
        ...
        return when {
            uri.contains("/episode") -> {
                val title = uri.substringAfter("$mainUrl/episode/").let { tt ->
                    ...
                    when {
                        !tt.contains("-season") && !tt.contains(Regex("-1x\\d+")) && !tt.contains("one-piece") -> "$fixTitle-season-${
                            Regex(
                                "-(\\d{1,2})x\\d+"
                            ).find(tt)?.groupValues?.getOrNull(1).toString()
                        }"
...
//                }
    ...
    private fun Element.toSearchResult(): AnimeSearchResponse {
        ...
        val epNum =
            this.select(".fixyear > h2.text-center").text().replace(Regex("[^0-9]"), "").trim()
                .toIntOrNull()
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        if (url.contains("movie") || url.contains("live-action")) {
            val mTitle =
                document.selectFirst(".sbox > .data > h1[itemprop = name]")?.text().toString()
                    .trim()
            val mPoster =
                document.selectFirst(".sbox > .imagen > .fix > img[itemprop = image]")
                    ?.attr("data-src")
